...
import 'package:tpk_soi8_staff/data/model.dart';
...
class SENDTOMANA extends GETSTAFFDATAEvent {}
...
class GETSTAFFDATABloc extends Bloc<GETSTAFFDATAEvent, List<unitdata>> {
  GETSTAFFDATABloc() : super([]) {
    ...
    on<SENDTOMANA>((event, emit) {
      return _SENDTOMANAFn([], emit);
    });
  }
...
//_SENDTOMANAFn
  Future<void> _SENDTOMANAFn(
      List<unitdata> toAdd, Emitter<List<unitdata>> emit) async {
    final responsepass = await Dio().post(
      server + "passtomana",
      data: {
        "poid": ACCEPTION.poid,
        "plant": ACCEPTION.plant,
      },
    );
    ...
    if (responsepass.statusCode == 200) {
      BlocProvider.of<BlocNotification>(MainBodyContext).UpdateNotification(
          "Success",
          "Success PO:${ACCEPTION.poid}",
          enumNotificationlist.Success);
      ACCEPTION.poid = '';
      ACCEPTION.plant = '';
    } else {
      BlocProvider.of<BlocNotification>(MainBodyContext).UpdateNotification(
          "ERROR", "Internet error", enumNotificationlist.Error);
    }
